...
make_boxplot <- function(df, df_name, y, y_name, ylims){
  ...
  ggplot(df) + 
    ...
    theme_pubr()+
    scale_fill_manual(values = c("Warm Years"="#95D055FF", "Cold Years"="#404788FF"))+
    labs(x=" ",
         y= " ",
         fill = " ")
}
...
make_contour_plot <- function(df, df_name, y, y_name, ylims){
  ...
  ggplot(df)+
    ...
    labs(title = " ",
         ...
         y = y_name,
         fill = " ")+
    scale_color_manual(values = c("Warm Years"="#95D055FF", "Cold Years"="#404788FF"))+
    theme_pubr()+
...
make_foursquare_plotlist <- function(fish_df, fish_name, ylimits_depth, ylimits_temp){
