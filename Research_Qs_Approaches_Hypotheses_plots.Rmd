...
# p_nrs_temp <- plot_contours(nrs, "nrs", gear_temperature, "gear_temperature")
# p_nrs_depth <- plot_contours(nrs, "nrs", bottom_depth, "bottom_depth")
yfs_temp <- make_cuts(yfs, "yfs", gear_temperature, "gear_temperature")
yfs_depth <- make_cuts(yfs, "yfs", bottom_depth, "bottom_depth")
remove(yfs)
p_yfs_temp <- make_contour_plot(yfs_temp, "yfs", gear_temperature, "gear_temperature")
p_yfs_depth <- make_contour_plot(yfs_depth, "yfs", bottom_depth, "bottom_depth")

```



```{r print-out-bin-plots, fig.height=8, fig.width=10}
# p_akp_temp
# p_akp_depth
# p_fhs_temp 
# p_fhs_depth
...
ggplot(yfs_temp)  +  
    stat_density2d(aes(x=length, y=gear_temperature))+
    theme_light()+
    ggtitle(paste("yfs", "gear_temperature"))+
    scale_fill_viridis()
```
