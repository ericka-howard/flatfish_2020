...
plot_contours <- function(df, df_name, y, y_name){
  ...
  p <- df %>%
    ...
    uncount(num) %>%
    ggplot()+
    geom_bin2d(aes(x=length, y=!!yvar))+
    theme_light()+
...
p_yfs_temp <- plot_contours(yfs, "yfs", gear_temperature, "gear_temperature")
p_yfs_depth <- plot_contours(yfs, "yfs", bottom_depth, "bottom_depth")
```

```{r print-out-plots, fig.height=12, fig.width=12}
p_akp_temp
p_akp_depth
p_fhs_temp 
p_fhs_depth
