...
ggplot()+
  geom_sf(data=grid)+
  geom_point(data=catch, aes(x=START_LONGITUDE, y=START_LATITUDE), color="blue", alpha=0.2)+
  theme_minimal()+
...
len_haul_expanded %>%
  ...
  ggplot()+
  geom_point(aes(x= LENGTH, y=n_HAUL, color=SPECIES_CODE), alpha=0.2)+
  facet_wrap(~YEAR)
```
...
station_lengths <- len_haul_expanded %>%
  group_by(STATIONID,SPECIES_CODE, SEX) %>%
  mutate(n_SPEC_STATION = sum(FREQUENCY))%>%
  group_by(STATIONID,SPECIES_CODE, LENGTH, SEX) %>%
...
```{r feel-like-this-works-CHECKINAM}
try_again <- station_lengths %>%
  group_by(STATIONID, SPECIES_CODE,LENGTH, SEX,YEAR) %>%
  summarise(PER_STAT_YEAR =sum(n_STATION))
```

```{r, visualizing,fig.height=14, fig.width=10}
try_again %>%
  filter(STATIONID=="A-02",
         SPECIES_CODE ==10130) %>%
  ggplot()+
  geom_point(aes(x=LENGTH, y=PER_STAT_YEAR))+
  facet_wrap(~YEAR)+
